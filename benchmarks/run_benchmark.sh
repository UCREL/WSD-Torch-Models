...
usage() {
    ...
    echo "  --device    Set device to use (default: cpu) can also be cuda for GPU"
...
for language in ${neural_tagger_languages[@]}; do
    echo "Running neural taggers for language: ${language}"
    for tagger_size in ${neural_tagger_sizes[@]}; do
        echo "Running neural tagger size: ${tagger_size}"
        uv run --no-group cpu --group cu128 ./benchmark_model.py \
            ${language} \
            ${tagger_size} \
...
uv run --no-group cpu --group cu128 ./format_benchmarking_data.py ${json_statistics_data_file}
