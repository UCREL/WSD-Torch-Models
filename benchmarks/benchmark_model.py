...
language_code_help = (
    "The language of the tagger to benchmark."
)
tagger_size_help = (
    "The size of the tagger to benchmark within the given `language`."
)
output_file_help = (
...
LANGUAGE_CODE_SIZE_TO_TAGGER_NAME = {
    "English_Small": "Neural-E-17M",
    "English_Base": "Neural-E-68M",
    "Multilingual_Small": "Neural-M-140M",
    "Multilingual_Base": "Neural-M-304M",
}


def main(language: benchmarking_utils.Languages = typer.Argument(help=language_code_help),
         tagger_size: benchmarking_utils.TaggerSizes = typer.Argument(help=tagger_size_help),
    ...
    * Loads the neural tagger for the specified language and tagger size.
    ...
    * Appends the benchmark statistics to the specified output file in JSON format.
    ...
    model_id = f"ucrelnlp/PyMUSAS-Neural-{language.value}-{tagger_size.value}-BEM"
    tagger_name = LANGUAGE_CODE_SIZE_TO_TAGGER_NAME[f"{language.value}_{tagger_size.value}"]
    ...
    output_statistics: dict[str, int | float | str] = {
        language_code_key: language.value,
        tagger_name_key: tagger_name,
    ...
    with benchmarking_utils.track_memory_usage(load_memory_requirements_key, load_gpu_requirements_key, output_statistics, device):
        neural_model, tokenizer = benchmarking_utils.load_tagger(model_id, device)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with benchmarking_utils.track_memory_usage(average_memory_required_key, average_gpu_memory_required_key, output_statistics, device):
            total_times = timeit.repeat(stmt='benchmarking_utils.tagger_speed_test(neural_model, tokenizer, Path(temp_dir), temp_file_prefix)',
